"""
GAD DXF vs cutting-sheet DXF validation (Run Comparison tab, "GAD vs DXF").

Ported from the standalone gad_baffle_pipeline.py (kept in the repo root,
untouched, for CLI use) -- the geometry logic below is the same. Pure
geometry, no LLM:

  1. Detect every capsule / stadium shape (two semicircular arcs joined by
     two straight sides) in the GAD DXF, and collapse them to sizes.
  2. Take the highest-count capsule size, find the smallest circle that
     encloses each instance, and export that circle + everything inside it
     as a standalone DXF.
  3. In the cutting-material DXF, find the "full baffle" (the radius that
     appears both as a full CIRCLE and as a cut ARC) and export it + its
     holes as a standalone DXF.
  4. Overlay the two (each shifted so its own outer circle sits at 0,0, no
     scaling), match every capsule to its nearest baffle hole, and write the
     overlay PNG + a diff JSON.
  5. Merge everything into one match JSON.

Entry points for the API layer: run_gad_dxf_validation() and
render_highlight(). Everything else is the original pipeline.

Differences from the standalone script: no argparse/CLI, and the two big
debug renders (whole-sheet capsule highlight, highest-count-group zoom) are
skipped by default -- the UI only needs the overlay PNG.
"""
...
def render_and_save_highest_count_group(doc, msp, best_key, results, png_path, json_path, render_png=True):
    """
...
def trace_and_diff_dxfs(gad_path, baffle_path, out_png, out_json, highlight_capsule_index=None):
    ...
    legend_handles = [
        Line2D([0], [0], color="red", lw=2, label="GAD capsules"),
        Line2D([0], [0], color="deepskyblue", lw=2, label="Cutting-sheet baffle holes"),
    ]
...
def run_pipeline(paths, render_debug_pngs=True):
    """Run the full pipeline using the resolved `paths` dict from build_paths().

    render_debug_pngs=False skips the two large debug renders (STEP 3's whole
    sheet, STEP 4's zoomed group); every JSON/DXF output and the STEP 7
    overlay are still produced.
    """
    ...
    if not capsules:
        raise RuntimeError(
            "No capsule/slot shapes were found in the GAD DXF "
            f"({semi_count} semicircular arcs seen, none joined into a capsule)."
        )

    export_capsule_sizes_json(capsules, paths["capsule_sizes_json"])
    if render_debug_pngs:
...
OUTPUT_PREFIX = "gadvalidation"   # fixed file prefix inside each run's output folder
RADIUS_MATCH_TOL = 0.05           # drawing units: hole/capsule radii within this count as "the same"


def is_gad_dxf_name(filename):
    """A DXF whose filename contains "GAD" (case and punctuation ignored) is
    the GAD drawing; any other DXF is treated as the cutting-material sheet.
    static/js/upload.js mirrors this rule -- keep in sync."""
    return "gad" in re.sub(r"[^a-z0-9]", "", (filename or "").lower())


def classify_match(capsule_radius, hole_radius, tol=RADIUS_MATCH_TOL):
    """MATCH if the matched hole's radius equals the capsule's radius, else MISMATCH."""
    if capsule_radius is None or hole_radius is None:
        return "MISMATCH"
    return "MATCH" if abs(capsule_radius - hole_radius) <= tol else "MISMATCH"


@dataclass
class GadDxfResult:
    merged: dict   # the merged capsule/baffle match JSON (see export_merged_match_json)
    paths: dict    # every path build_paths() resolved (outputs live under paths["output_dir"])


def run_gad_dxf_validation(gad_dxf, cutting_dxf, output_dir):
    """Run the whole pipeline on a GAD DXF + cutting-sheet DXF, writing
    everything under `output_dir`. Raises RuntimeError with a readable
    message on failure (no capsules found, no full baffle found, ...)."""
    paths = build_paths(str(gad_dxf), str(cutting_dxf), output_dir=str(output_dir), prefix=OUTPUT_PREFIX)
    run_pipeline(paths, render_debug_pngs=False)
    with open(paths["merged_match_json"], encoding="utf-8") as f:
        merged = json.load(f)
    return GadDxfResult(merged=merged, paths=paths)


def paths_for_output_dir(output_dir):
    """Rebuild the paths dict for an existing run's output folder (the input
    DXF paths are irrelevant here -- only output/extract file locations are used)."""
    return build_paths("", "", output_dir=str(output_dir), prefix=OUTPUT_PREFIX)


def render_highlight(output_dir, capsule_index, out_png):
    """Re-run just STEP 7 for an existing run with `capsule_index` ringed in
    yellow (same as the Streamlit app's "Highlight in image")."""
    paths = paths_for_output_dir(output_dir)
    out_json = os.path.splitext(str(out_png))[0] + ".json"
    trace_and_diff_dxfs(paths["circle_extract_dxf"], paths["baffle_dxf"], str(out_png), out_json,
